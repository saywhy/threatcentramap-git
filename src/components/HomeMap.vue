    ...
    <div class="homemap">
        ...
        <div>
            <div class="attack-source">
                <div class="attackMes-title">
                    <i>威胁源</i>
                </div>
                <table class="attack-table">
                    <tr v-for="item in country_top">
                        <td style="width:35%">
                            <span class="flg" :class="'flg-'+item[0]"></span>
                            <span v-bind="item[2]" class="country-name"></span>
                        </td>
                        <td>
                            <div class="process-outline">
                                <div class="process-line" :style="{width: item[3]}" v-bind="item[1]"></div>
                            </div>
                        </td>
                    </tr>
                </table>
                ...
                <div class="footer-bg">
                    <div class="attack-event">
                        ...
                        <table class="attack-table">
                            <tr>
                                ...
                                <th style="width:33%">威胁</th>
                                <th style="width:10%">威胁源</th>
                                <th style="width:15%">威胁类型</th>
                                <th style="width:13%">受害者</th>
                                <th style="width:18%">网内首次发现时间</th>
                            </tr>
                            <!-- <tr :class-odd="'mycolor'" ng-class-even="'mycolor2'" ng-repeat="item in attackList"> -->
                            <tr v-for="item in attackList">
                                <td v-text="item.time"></td>
                                <td v-text="item.indicator"></td>
                                <td v-text="item.src_country_name"></td>
                                <td v-text="typeNames[item.type]"></td>
                                <td v-text="item.dst_ip"></td>
                                <td v-text="item.first_seen"></td>
                            </tr>
                        </table>
                    </div>
                    <div class="attack-target">
                        ...
                        <div class="attack-map">
                            <div id="container">
                                <div echarts-angular option="option" style="height:14.5vw; width:75%;"></div>
                            </div>
                            <ul class="attack-target-legend" style="margin-top:19px; right:1vw;">
                                <liv-for="item in dst_top">
                                    <span v-text="item.value[2]"></span>
                    ...
                    <div class="attack-type">
                        ...
                        <div class="attack-type-map">
                            <div>
                                <div echarts-angular option="chart_category" style="height:14.5vw; width:75%; padding-top:10px;"></div>
                            </div>
                            <ul class="attack-target-legend" id="attack-category-legend"></ul>
                        </div>
                    </div>
                </div>
            </div>
...
import 'common/js/d3.v4.min'
import 'common/js/topojson.v3'
import 'common/js/marks'
import 'common/js/leaflet'
import 'common/js/flight'
import 'common/js/victor.min'
import 'common/js/moment.min'
...
export default {
    ...
    methods: {
        d3map() {
            ...
            d3.json("./static/json/world_map.json",
                function (a, b) {
                    function removeAll() {
                    ...
                    function getIntelligence() {
                        ...
                        var updateIntelligence = new Process(function () {
                            d3.json("./static/json/intelligence/" + index + ".json", function (e, data) {
                    ...
                    function getAttack() {
                        d3.json("./static/json/attack.json", function (e, data) {
                            var index = 0;
                            ...
                            var readAttack = new Process(function () {
                                ...
                                if (Math.random() < chance) {
                                    ...
                                    var atkGroup = [];
                                    for (var i = 0; i < num; i++) {
                                        var item = this.attackList[index];
                                        var atk = {
                                        ...
                                        index++;
                                        if (index >= this.attackList.length) {
                                            index = 0;
                                        }
                                    }
                                    attack.add(atkGroup);
                    ...
                    d3.json("./static/json/province_coordinates.json", function (e, data) {
                        console.log(222);
        ...
        init() {
            for (var i = 0; i < 7; i++) {
                this.attackList.push({ indicator: '　' });
            }
        ...
        readCountry(List) {
            ...
            for (var i = 0; i < 7; i++) {
                ...
                this.country_top.push(List[i]);
            }
            console.log(this.country_top);

        },
        pushToAttackList(atc) {
            this.attackList.push(atc);
            if (this.attackList.length > 7) {
                this.attackList.shift();
            }
            this.dst_count(atc.dst_subdivisions_name);
            setTimeout(function () {
                this.setDst_top();
            }, 0);
        },
        ...
        dst_count(name) {
            if (this.dst_top_all[name]) {
                this.dst_top_all[name].value[2]++;
            }
            var value = this.dst_top_all[name].value[2];
            var isTOP1 = true;
            for (var i = this.dst_top.length - 1; i >= 0; i--) {
                var item = this.dst_top[i];
                if (item.name == name) {
                    this.dst_top.splice(i, 1);
                    continue;
                }
                if (item.value[2] >= value) {
                    this.dst_top.splice(i + 1, 0, this.dst_top_all[name]);
                    isTOP1 = false;
                    break;
                }
            }
            if (isTOP1) {
                this.dst_top.splice(0, 0, this.dst_top_all[name]);
            }
            if (this.dst_top.length > 7) {
                this.dst_top.pop();
            }
        }
    },
...
@import "../common/css/flg.css";
@import "../common/css/plane.css";
.homemap {
  width: 100%;
  height: 100%;
  color: #000;
